    ...
    <div class="container">
        ...
        {foreach $domains as $count => $domain}
            <div class="row align-items-center">
                ...
                {if $count == 0}
                <div class="col-md-2 cust-doamin-head">
                    Domain
                </div>
                <div class="col-md-10 cust-doamin-head">
                    Services
                </div>
                {/if}

                <div class="col-md-2 domain-left-box">
                    <div class="wrapper-domain-left">
                        <h2 class="selected_domain_name" id="domain_{$count}" data-id="{$count}">{$domain['domain']}</h2>
                    </div>
                </div>
                <div class="col-md-10">
                    <div class="wrapper">
                                     ...
                                     {foreach $groupProductArray as $key => $groupProduct}
                                      <tr class="buy-row">
                                         <td class="cell" data-title="Services">
                                            <span class="text-wr"><input id="GridView2_ctl0{$key}_{$count}_CheckBox2" class="check_product" type="checkbox" data-domain="{$count}" data-id="{$groupProduct['gid']}" name="GridView2$ctl0{$key}_{$count}$CheckBox2"><label for="GridView2_ctl0{$key}_{$count}_CheckBox2">{$groupProduct['group_name']}</label></span>
                                         </td>
                                         <td class="cell" data-title="Plan">
                                            <select class="domain_select select_product_plan" id="select_product_plan_{$groupProduct['gid']}">
                                               ...
                                               <option data-price="{$product['pricing'][{$currency->code}]['monthly']}" data-period="monthly" value="{$product['pid']}">{$product['name']}</option>
                                               {/foreach}
                                            </select>
                                         </td>
                                         <td class="cell" data-title="Quantity">
                                            <select id="GridView2_ctl0{$key}_{$count}_Quantity" class="domain_select select_product_quantity">
                                         ...
                                         <td class="cell" data-title="Subscription Period">
                                            <select name="GridView2$ctl0{$key}_{$count}$SubscriptionPeriod" id="GridView2_ctl0{$groupProduct['gid']}_SubscriptionPeriod" class="domain_select select_product_period">
                                               ...
                                               <option data-price="" value="annually">Annually</option>
                                         ...
                                         <td class="cell" data-title="Price">
                                            <span id="GridView2_ctl0{$key}_{$count}_lblUnitPrice" class="show_product_plan select_price_{$count}_{$groupProduct['gid']}" value="{$groupProduct['products'][0]['pricing'][{$currency->code}]['monthly']}">{$groupProduct['products'][0]['pricing'][{$currency->code}]['monthly']} {$currency->code}</span>
                                         </td>
                                      </tr>
                                    {/foreach}
                                 </table>
        ...
        <div class="row">
            <input type="hidden" value="{$currency->code}" class="currency-code">
            <div class="col-md-12  buy-total">
                <div class="price-grid my-4">
                    {$LANG.getSericeTotal}: <span class="total_cart_price"></span>
                </div>
            </div>
            <div class="col-md-12 p-sub text-center">
 ...
 <script>
   $(document).ready(function () {
        ...
        $('.submit_product').on('click', function () {
            ...
            Object.entries(groupIds).forEach(([domain, id]) => {
                var pid = $('#select_product_plan_'+id).find('option:selected').val();
                var billing_period = $('#GridView2_ctl0'+id+'_SubscriptionPeriod').find('option:selected').val();
                var domain_name =  $('#domain_'+domain).text();
                if(domain_name){
            ...
            $.ajax({
                url: '',
                type: 'POST',
                data: { productIds: productIds, billing_cycles:billing_cycles, action: "insert_products", ajaxcall: true },
        ...
        function updateTotalPrice() {
            ...
            Object.entries(selectedIds).forEach(([domain, id]) => {
                var total_price = $('.select_price_' + domain + '_' + id).attr('value');
